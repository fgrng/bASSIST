  ...
  scope :datatables, -> {
    visible.
      ...
      joins("LEFT OUTER JOIN exercises AS dt_exercises ON dt_exercises.id = submissions.exercise_id").
			joins("LEFT OUTER JOIN lsa_scorings AS dt_lsa_scorings ON dt_lsa_scorings.submission_id = submissions.id AND dt_lsa_scorings.updated_at = (SELECT MAX(updated_at) FROM lsa_scorings WHERE lsa_scorings.submission_id = submissions.id)").
      select("submissions.*").
      ...
      select("dt_feedbacks.passed AS dt_feedbacks_passed").
			select("dt_lsa_scorings.grade AS dt_lsa_scorings_grade")
  }
